...
class FieldCheckboxes extends Field
{
	...
	public function render()
	{
		...
		?>
		<div class="csframework-field csframework-field-checkboxes<?php echo esc_attr( $this->_depend ? ' csframework-depend-field' : '' );  ?>"<?php echo ( bool ) $this->_depend ? ' data-depend="' . esc_attr( implode( ';', $this->getDependecies() ) ) . '"' : '';  ?>>
			...
			<?php $i=0; foreach ( $this->_values as $value => $label ): ?>
				<label for="<?php echo esc_attr( $this->getInputId() . '-' . $i ); ?>" class="checkbox-label <?php echo esc_attr( $this->_class ); ?>">
					<input type="checkbox" name="<?php echo esc_attr( $this->getInputName() ); ?>[]" id="<?php echo esc_attr( $this->getInputId() . '-' . $i ); ?>" value="<?php echo esc_attr( $value ); ?>" <?php checked( in_array( $value, is_array( $this->_value ) ? $this->_value : $this->_default, true ), true ); ?> class="<?php echo esc_attr( $this->_class ); ?>" />
