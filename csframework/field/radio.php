...
class FieldRadio extends Field
{
	...
	{
		?>
		<div class="csframework-field csframework-field-radio<?php echo esc_attr( $this->_depend ? ' depend-field' : '' );  ?>"<?php echo ( bool ) $this->_depend ? ' data-depend="' . esc_attr( implode( ';', $this->getDependecies() ) ) . '"' : '';  ?>>
			...
			<?php $i=0; foreach ( $this->_values as $value => $label ): ?>
				<label for="<?php echo esc_attr( $this->getInputId() . '-' . $i ); ?>" class="csframework-radio-label <?php echo esc_attr( $this->_class ); ?>">
					<input type="radio" name="<?php echo esc_attr( $this->_app->getFieldsVar() . $this->getInputName() ); ?>" id="<?php echo esc_attr( $this->getInputId() . '-' . $i ); ?>" value="<?php echo esc_attr( $value ); ?>" <?php checked( $value, !is_null( $this->_value ) && !empty( $this->_value ) ? $this->_value : $this->_default ); ?>>
