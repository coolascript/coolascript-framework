...
class Csframework
{
	 ...
	 * @var string
	 */
	protected $_fields_var = 'csframework';
	/**
	...
	/**
	 * Retrieve the key for fields that generated by your plugin (theme)
	 * @return string
	 */
			...
			if ( file_exists( $dir . 'taxonomy' ) && $handle = opendir( $dir . 'taxonomy' ) ) {
				while ( false !== ( $entry = readdir( $handle ) ) ) {
					if ( is_file( $dir . 'taxonomy/' . $entry ) ) {
						...
						if ( class_exists( $class_name ) ) {
							$class_name::getInstance()->setFieldsBase( $this->_fields_var )->addFields();
						}
					}
				}
				closedir( $handle );
	...
	/**
	 * WP plugins_loaded action trigger to localize the plugin
	 * @return void
	 */
	public function csflocalize()
	{
		load_plugin_textdomain( 'coolascript-framework', false, plugin_basename( CSFRAMEWORK_PLUGIN_DIR ) . '/languages' );
	}
	...
	public function run()
	{
		...
		add_action( 'plugins_loaded', array( $this, 'csflocalize' ) );
		return $this;
	}
